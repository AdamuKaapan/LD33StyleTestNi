...
public class MenuManager {
	...
	public static void initialize() {
		...
		HvlLabeledButton defaultButton = new HvlLabeledButton(256, 64, new HvlTextureDrawable(Main.getTexture(Main.buttonIndex)), new HvlTextureDrawable(
				Main.getTexture(Main.buttonHoverIndex)), new HvlTextureDrawable(Main.getTexture(Main.buttonPressedIndex)), fontSmall, "no text d:", 2f,
				Color.white);
		...
		main.getFirstChildOfType(HvlArrangerBox.class).add(
				new HvlLabeledButton.Builder().setText("play").setClickedCommand(new HvlButtonMenuLink(levels)).build());
		...
		levels.add(new HvlLabeledButton.Builder().setText("previous").setClickedCommand(new HvlAction1<HvlButton>() {
			@Override
			public void run(HvlButton arg0Arg) {
				if (Game.levels.indexOf(Game.getCurrentLevel()) <= 0)
					Game.setCurrentLevel(Game.levels.get(Game.levels.size() - 1));
				else
					Game.setCurrentLevel(Game.levels.get(Game.levels.indexOf(Game.getCurrentLevel()) - 1));
				updateMinimap();
			}
		...
		levels.add(new HvlLabeledButton.Builder().setText("next").setClickedCommand(new HvlAction1<HvlButton>() {
			@Override
			public void run(HvlButton arg0Arg) {
				if (Game.levels.indexOf(Game.getCurrentLevel()) + 1 >= Game.levels.size())
					Game.setCurrentLevel(Game.levels.get(0));
				else
					Game.setCurrentLevel(Game.levels.get(Game.levels.indexOf(Game.getCurrentLevel()) + 1));
				updateMinimap();
			}
		...
		levels.add(new HvlDrawableComponent(512, 512, new HvlComponentDrawable() {
			...
			public void draw(float x, float y, float w, float h, float delta) {
				...
				// hvlDrawQuad((Display.getWidth()/2) - 256,
				// (Display.getHeight()/2) - 256, 512, 512, Color.white);
			}
		}));
